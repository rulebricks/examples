...
if __name__ == "__main__":
    ...
    for test_case in test_cases:
        try:
            outcome = rb.rules.solve(slug=rule.slug, request=test_case)
        except Exception as e:
            print(f"Failed to solve test case: {test_case}")
            print(e)
    single_end_time = time()
    print(
        f"Solved {num_test_cases_single} rules individually in {single_end_time - single_start_time:.2f} seconds"
    )
    ...
    outcomes = rb.rules.bulk_solve(slug=rule.slug, request=test_cases)
    batch_end_time = time()
    print(
        f"Solved {num_test_cases_batch} rules in a single batch in {batch_end_time - batch_start_time:.2f} seconds"
    )

    print(
        "Avg time per rule (individual):",
        (single_end_time - single_start_time) / num_test_cases_single,
        "seconds",
    )
    print(
        "Avg time per rule (batch):",
        (batch_end_time - batch_start_time) / num_test_cases_batch,
        "seconds",
    )
